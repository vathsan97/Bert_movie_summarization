...
def format_to_lines(args):
    # corpus_mapping = {}
    lis = glob.glob("/proj/sund778/work/bert_movies/bert_tokenized/*json")
    random.shuffle(lis)
    # print(temp)
    train_files = lis[:int(0.8*len(lis))]
    valid_files = lis[int(0.8*len(lis)) : int(0.9*len(lis))]
    test_files = lis[int(0.9*len(lis)):]
